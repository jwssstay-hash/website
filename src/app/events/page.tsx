            ...
            {events.map((event, index) => (
              <div key={event.id} className="bg-woodside-900/60 backdrop-blur-xl border border-white/10 rounded-3xl overflow-hidden shadow-2xl flex flex-col group hover:bg-woodside-900/80 transition-all hover:-translate-y-2 duration-500" style={{ animationDelay: `${index * 100}ms` }}>
                ...
                <div className="p-8 flex-grow flex flex-col relative z-10 -mt-12">
                  ...
                  <a href={`/booking?category=Event&stay=${encodeURIComponent(event.title || event.description?.substring(0, 30) || 'Special Event')}&date=${encodeURIComponent(event.date || '')}`} className="mt-6 w-full bg-white text-woodside-950 font-bold py-3 rounded-xl hover:bg-woodside-100 transition-colors shadow-lg text-center flex items-center justify-center">
                    Book this Event
                  </a>
                </div>
              </div>
